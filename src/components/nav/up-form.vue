...
<template>
<div>
  <el-dialog v-draggable="dialogDraggableOptions" :visible.sync="dialog" title="添加外部系统" :modal-append-to-body="true" :append-to-body="true" size="middle--fixed" :modal="true" :close-on-click-modal="false" :before-close="closeEvent">
      ...
      <el-row :gutter="20">
        <el-col :span="24">
          <el-form-item label="系统名称">
            <el-input placeholder="请输入系统名称" v-model="editForm.systemName"></el-input>
          </el-form-item>
        </el-col> 
        <el-col :span="24">
          <el-form-item label="系统地址">
            <el-input placeholder="请输入系统地址" v-model="editForm.systemUrl"></el-input>
          </el-form-item>
        </el-col> 
        <el-col :span="24">
         ...
         <el-col :span="24">
          <el-form-item label="">
            <img :src="fileAdress" style="" :class="fileAdress === '' ? '' : 'reviewImg'"></img>
          </el-form-item>
        </el-col> 
        <!-- <el-col :span="24">
...
import { OUT_SYETEM_SAVE } from '../../store/types'
export default {
  mixins: [mixin],
  components: {
    upload
  ...
  methods: {
    ...
    submit() {
      ...
      if(this.editForm.systemName === '' || this.editForm.systemName === undefined) {
        this.$alert('系统名称不能为空', {
          confirmButtonText: '确定',
          type: 'warning'
        })
        return;
      }else if(this.editForm.systemUrl === '' || this.editForm.systemUrl === undefined) {
        this.$alert('系统地址不能为空', {
          confirmButtonText: '确定',
          type: 'warning'
        })
      ...
      if (this.editForm.fileUrlId !== '' && this.editForm.fileUrlId !== undefined) {
        this.file.systemPic = this.editForm.fileUrlId;
        this.file.systemUrl = this.editForm.systemUrl;
        this.file.systemName = this.editForm.systemName;
        // this.file.Name = this.editForm.fileName + '.' + this.editForm.fileSuffix;
        this.dispatch(OUT_SYETEM_SAVE, this.file).then(() => {
          this.$message({
            type: 'success',
    ...
    uploadSubmit(f) {
      // 获取当前上传文件列表文件id
      this.editForm.fileUrlId = '';
      this.sendData = [];
      console.log(f.data.res.Data.FileUrl, 'this.fffffff')
      this.fileAdress = f.data.res.Data.FileUrl;
