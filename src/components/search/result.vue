...
<el-layout class="app-layout result-item" :fit="true">
  ...
  <el-region class="app-north" slot="north" :zoom="false" type="north" :fixed="false">
    <top-bar>
      <div slot="search">
        <span class="sub-title">通用搜索</span>
        <div class="search-input" @keyup.13="searchWord">
          <!-- <el-input style="width: 320px" v-model="inputValue">
              <el-button slot="append" icon="search">搜索</el-button>
            </el-input> -->
          <el-input style="width: 446px" v-model="inputValue">
            <el-select v-model="selectValue" slot="prepend" width="130px">
              <el-option v-for="(item,i) in selectOption" :label="item.label" :value="item.value" :key="i"></el-option>
            </el-select>
            <el-button slot="append" icon="search" @click="searchWord" class="search-btn"></el-button>
          </el-input>
        </div>
      </div>
    </top-bar>
  ...
  <el-region :class="{'app-west': true, 'app-west--mini':miniWest }" slot="west" type="west" :zoom="false" :fixed="false">
    ...
    <!-- @on-match-keyword="colOnClickEvent" -->
    ...
    <!-- <div id="top" class="scrollTop-icon" @click="backTop()"> -->
    <!-- <i class="iconfont icon-zhiding"></i><br/> -->
    <!-- 回顶部 -->
    <!-- </div> -->
  ...
  <el-region class="app-center" slot="center" type="center">
    ...
    <div class="search-wrap">
      ...
      <div class="app-toolbar app-toolbar--search clearfix rel" v-if="treeRes!==null">
        <el-row>
          <!-- <wel-col class="query-box" :span="miniModel?11:7"> -->
          <el-col class="query-box" :span="12">
            <div class="query-box__item" :title="tabs[currentTab].keyword">
              <label class="query-box__item__name">条件</label> {{tabs[currentTab].keyword}}
            </div>
          ...
          <el-col :class="{'ctrl-box':true,'ctrl-box--mini':miniModel}" :span="12">
            ...
            <!-- <el-button-group>
              ...
              <!--<el-button :class="{action:showRightBar===true}" v-show="type!==2" type="" title="筛选"-->
              <!--@click="switchToolbar">-->
      ...
      <div class="app-content">
        <!-- 列表 -->
        <!-- v-loading.body="loading"  listType===2-->
        <el-table v-if="listType===1" v-loadmore="loadMore" id="searchResTable" class="app-table" :show-header="false" :data="resList" :border="true" style="width:100%;" :height="tableHeights" :row-class-name="rowClassName" @selection-change="updateTableSelection">
          <el-table-column class-name="compact user-bg" label="内容集">
            <template scope="scope">
              <!-- <div :class="{compact__wrap:true, 'compact__wrap--grid':listType===2}"> -->
              <div :class="{compact__wrap:true}">
                ...
                <div class="compact__box">
                  <div class="compact__title">
                    ...
                    <!-- <el-button type="text" size="small" v-if="checkRule('Notice/AddShare',true)===true">
                      <i class="iconfont icon-share" title="分享" @click="share(scope.row)"></i>
                    </el-button> -->
                    ...
                    <!-- <el-button type="text" size="small" @click="exportSingle(scope.row['_id'])" title="导出数据">
...
<!--&lt;!&ndash; <span>技术支持：珠海市新德汇信息技术有限公司</span> &ndash;&gt;-->
<!--</el-region>-->
</el-layout>
</template>
...
<script>
  ...
  import { SEARCH_RESULT, SEARCH_AGGR_RESULT, GET_SEARCH_ELM_LIST } from '../../store/types'
  ...
  export default {
    ...
    watch: {
      ...
      scrollCount(oldVal, newVal) {
        ...
        if (this.isAll === false && obj.eq(obj.length - 1).offset().top - this.$('.search__table-box').height() <= 150) {
          if (this.loading === false) {
            this.tabs[this.currentTab].pagenav.currentpage++;
            this.fetchList()
          }
        }
      }
    ...
    methods: {
      secondSearch() {
        ...
        let pagenav = { currentpage: 1, limit: 20 }
      ...
      fetchList(pagenav) {
        ...
        this.showTips = false
        this.maskLoading = !this.fromTrigger
        ...
        let currentTab = this.tabs[this.currentTab],
          ...
          queryObj
        if (currentpage === 1 || (pagenav || {}).index > 0) {
          this.nextQuery = {
        ...
        if (this.type === 2) {
          ...
          // 页签只在右键二级检索时才使用, 另不更新左聚类树 by Eeqlee @20171110
          if (currentpage === 1) {
        ...
        if (this.secondFlag) {
          if (this.secondKeys) {
            queryObj.SearchWhere[0].Keyword = queryObj.SearchWhere[0].Keyword.split('+')[0] + '+' + this.secondKeys
          } else {
            queryObj.SearchWhere[0].Keyword = queryObj.SearchWhere[0].Keyword.split('+')[0]
          }
        }
        // 读取接口
      ...
      handleSingleData() {
        ...
        // useMilliesecond
        if (this.useMilliesecond === null) this.useMilliesecond = this.search.searchResult.UseMilliesecond
        ...
        this.columns = this.search.searchResult.TableInfo.Columns.map(c => {
          return {
            ...
            disabled: c.IsSortFlag === '0',
            isDefault: c.IsDefault === '1'
          }
        })
        // d.Source['ngw_table_sign'] = d.TableInfo.ID
        this.exportTabID = this.search.searchResult.TableInfo.ID
        let ruslList = [...this.search.searchResult.Source]
        ruslList.forEach(d => {
          d.flag = false
        })
        this.list = [...this.list, ...ruslList]
        // 更新历史
      ...
      updateCat(val, formRightbar, options) {
        ...
        this.fromTrigger = !!val.fromTrigger
        this.useMilliesecond = null
        this.fetchList(pagenav)
      },
      /**
       * [addTemp 添加临时储存]
    ...
    mounted() {
      // 获取要素集合
      this.dispatch(GET_SEARCH_ELM_LIST).then(() => {
        if (this.search.searchElmList && this.search.searchElmList.length > 0) {
          this.search.searchElmList.map(d => {
            this.selectOption.push({ value: parseInt(d.ID), label: d.Name })
          })
        }
      })
      if (!get('ngw_search_searchWhere_' + this.$route.query.key) && this.$route.query.key) {
        let formData = {
          SearchWhere: [{
            Keyword: this.$route.query.key,
            KeywordType: 0,
            Name: this.$route.query.type === 0 ? '全部' : '要素',
        ...
        save('ngw_search_searchWhere_' + this.$route.query.key, formData)
      }
      this.inputValue = this.$route.query.key
      this.selectValue = parseInt(this.$route.query.type) || this.selectValue
      this.treeQuery = get('ngw_search_searchWhere_' + this.$route.query.key)
      this.updateKeywords([this.$route.query.key])
      this.fitEvent()
      // 初始化页签
      this.tabs.push({
        keyword: this.treeQuery.SearchWhere.map(d => d.Keyword).join('_'),
      ...
      window.onmousewheel = function() {
        if (self.listType === 2) {
          ...
          console.log(self.scrollCount, self.$('#searchResTable').height(), 'self.scrollCount 44444444444444')
        }
      }
