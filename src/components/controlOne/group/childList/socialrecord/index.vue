...
<div style="height: 100%;">
  ...
  <tableList ref='tablelist' title="社会履历列表" :tab="table.shll" :list="pertionnelInfo" :pagenav="pageInfo" :edit="checkRule('fxpcgk/socialrecord/update',basePeople)" :countLength="pageInfo.count" :readonly="readonly" :isEnd="isEnd" :remove="checkRule('fxpcgk/socialrecord/delete',basePeople)"
...
import {
  ...
  FXPCGK_SOCIALRECORD_INFO,
  FXPCGK_SOCIALRECORD_SAVE,
  FXPCGK_SOCIALRECORD_UPDATE,
  FXPCGK_SOCIALRECORD_DELETE,
  FXPCGK_SOCIALRECORD_LIST
} from '../../../../../store/types'
...
export default {
  ...
  methods: {
    queryList() {
      if(this.mainId) {
        // 查询列表
        this.dispatch(FXPCGK_SOCIALRECORD_LIST, {
          fjid: this.mainId,
          PageIndex: this.pageInfo.currentpage,
          PageSize: this.pageInfo.limit
        }).then(() => {
          this.pertionnelInfo = this.focusPerson.fxpcgkSocialrecordList.PageData
          this.pageInfo.count = this.focusPerson.fxpcgkSocialrecordList.DataCount
          this.isEnd = true
        })
    ...
    getQueryInfo(rowId, callback = () => {}) {
      this.dispatch(FXPCGK_SOCIALRECORD_INFO, {
        id: rowId
      }).then(() => {
        callback(this.focusPerson.fxpcgkSocialrecordInfo)
      })
    },
    /**
     * 新增按钮事件
    ...
    remove(obj) {
      this.$confirm('确定要删除此项?', '温馨提示', {
        ...
        this.dispatch(FXPCGK_SOCIALRECORD_DELETE, {
          ids: obj.row.id
        }, obj.index).then(() => {
          this.$message({
            message: '删除成功！',
    ...
    submit(form) {
      // 表单数据提交
      let submitType = FXPCGK_SOCIALRECORD_SAVE
      if(form.id !== '') { // 判断表单id是否为空，非空可判断为更新提交
        submitType = FXPCGK_SOCIALRECORD_UPDATE
      }
