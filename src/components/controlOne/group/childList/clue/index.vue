...
<div style="height: 100%;">
  ...
  <tableList ref='tablelist' title="线索信息采集列表" :tab="table.xsxx" :list="pertionnelInfo" :pagenav="pageInfo" :edit="checkRule('fxpcgk/clue/update',basePeople)" :countLength="pageInfo.count" :readonly="readonly" :isEnd="isEnd" :remove="checkRule('fxpcgk/clue/delete',basePeople)"
...
export default {
  ...
  methods: {
    queryList() {
      if(this.mainId) {
        // 查询列表
        this.dispatch(FXPCGK_CLUE_LIST, {
          fjid: this.mainId,
          PageIndex: this.pageInfo.currentpage,
          PageSize: this.pageInfo.limit
        }).then(() => {
          this.pertionnelInfo = this.focusPerson.fxpcgkClueList.PageData
          this.pageInfo.count = this.focusPerson.fxpcgkClueList.DataCount
          this.isEnd = true
        })
    ...
    getQueryInfo(rowId, callback = () => {}) {
      this.dispatch(FXPCGK_CLUE_INFO, {
        id: rowId
      }).then(() => {
        callback(this.focusPerson.fxpcgkClueInfo)
      })
    },
    /**
     * 新增按钮事件
    ...
    remove(obj) {
      this.$confirm('确定要删除此项?', '温馨提示', {
        ...
        this.dispatch(FXPCGK_CLUE_DELETE, {
          ids: obj.row.id
        }, obj.index).then(() => {
          this.$message({
            message: '删除成功！',
    ...
    submit(form) {
      // 表单数据提交
      let submitType = FXPCGK_CLUE_SAVE
      if(form.id !== '') { // 判断表单id是否为空，非空可判断为更新提交
        submitType = FXPCGK_CLUE_UPDATE
      }
      form.qqlx = '1'
