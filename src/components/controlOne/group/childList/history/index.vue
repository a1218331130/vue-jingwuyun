...
<div style="height: 100%;">
  ...
  <tableList ref='tablelist' title="历史上访" :tab="table.lssf" :list="pertionnelInfo" :pagenav="pageInfo" :isEnd="isEnd" :edit="checkRule('fxpcgk/solpethis/update',basePeople)" :readonly="readonly" :remove="checkRule('fxpcgk/solpethis/delete',basePeople)"
...
import {
  ...
  FXPCGK_SOLPETHIS_INFO,
  FXPCGK_SOLPETHIS_SAVE,
  FXPCGK_SOLPETHIS_UPDATE,
  FXPCGK_SOLPETHIS_DELETE,
  FXPCGK_SOLPETHIS_LIST
} from '../../../../../store/types'
...
export default {
  ...
  methods: {
    queryList() {
      if(this.mainId) {
        // 查询列表
        this.dispatch(FXPCGK_SOLPETHIS_LIST, {
          fjid: this.mainId,
          PageIndex: this.pageInfo.currentpage,
          PageSize: this.pageInfo.limit
        }).then(() => {
          this.pertionnelInfo = this.focusPerson.fxpcgkSolpethisList.PageData
          this.pageInfo.count = this.focusPerson.fxpcgkSolpethisList.DataCount
          this.isEnd = true
        })
    ...
    getQueryInfo(rowId, callback = () => {}) {
      this.dispatch(FXPCGK_SOLPETHIS_INFO, {
        id: rowId
      }).then(() => {
        callback(this.focusPerson.fxpcgkSolpethisInfo)
      })
    },
    /**
     * 新增按钮事件
    ...
    remove(obj) {
      this.$confirm('确定要删除此项?', '温馨提示', {
        ...
        this.dispatch(FXPCGK_SOLPETHIS_DELETE, {
          ids: obj.row.id
        }, obj.index).then(() => {
          this.$message({
            message: '删除成功！',
    ...
    submit(form) {
      // 表单数据提交
      let submitType = FXPCGK_SOLPETHIS_SAVE
      if(form.id !== '') { // 判断表单id是否为空，非空可判断为更新提交
        submitType = FXPCGK_SOLPETHIS_UPDATE
      }
