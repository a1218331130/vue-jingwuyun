...
<div style="height: 100%;">
  ...
  <tableList ref='tablelist' title="处罚处分信息列表" :tab="table.cfcf" :list="pertionnelInfo" :pagenav="pageInfo" :edit="checkRule('fxpcgk/punish/update',basePeople)" :countLength="pageInfo.count" :readonly="readonly" :isEnd="isEnd" :remove="checkRule('fxpcgk/punish/delete',basePeople)"
...
import {
  ...
  FXPCGK_PUNISH_INFO,
  FXPCGK_PUNISH_SAVE,
  FXPCGK_PUNISH_UPDATE,
  FXPCGK_PUNISH_DELETE,
  FXPCGK_PUNISH_LIST
} from '../../../../../store/types'
...
export default {
  ...
  methods: {
    queryList() {
      if(this.mainId) {
        // 查询列表
        this.dispatch(FXPCGK_PUNISH_LIST, {
          fjid: this.mainId,
          PageIndex: this.pageInfo.currentpage,
          PageSize: this.pageInfo.limit
        }).then(() => {
          this.pertionnelInfo = this.focusPerson.fxpcgkPunishList.PageData
          this.pageInfo.count = this.focusPerson.fxpcgkPunishList.DataCount
          this.isEnd = true
        })
    ...
    getQueryInfo(rowId, callback = () => {}) {
      this.dispatch(FXPCGK_PUNISH_INFO, {
        id: rowId
      }).then(() => {
        callback(this.focusPerson.fxpcgkPunishInfo)
      })
    },
    /**
     * 新增按钮事件
    ...
    remove(obj) {
      this.$confirm('确定要删除此项?', '温馨提示', {
        ...
        this.dispatch(FXPCGK_PUNISH_DELETE, {
          ids: obj.row.id
        }, obj.index).then(() => {
          this.$message({
            message: '删除成功！',
    ...
    submit(form) {
      // 表单数据提交
      let submitType = FXPCGK_PUNISH_SAVE
      if(form.id !== '') { // 判断表单id是否为空，非空可判断为更新提交
        submitType = FXPCGK_PUNISH_UPDATE
      }
