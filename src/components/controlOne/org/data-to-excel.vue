...
<el-dialog v-draggable="dialogDraggableOptions" title="数据导出" v-loading="downloading" :visible.sync="dataOutdialog" @open="initDialog" :modal="true" :close-on-click-modal="false" :before-close="closeEvent">
  ...
  <form ref="postform" method="post" :action="rod+'fxpcgk/indorg/toexcel'">
  ...
  export default {
    ...
    methods: {
      ...
      initDialog() {
        ...
        this.checkt = false
        this.formProp = []
        this.formData = [{ prop: 'ZDHYMC', label: '行业名称' },
          { prop: 'ZDHYLBMC', label: '重点行业类别' },
          { prop: 'HYDH', label: '行业电话' },
          { prop: 'XXDZ', label: '详细地址' },
          { prop: 'JD', label: '经度' },
      ...
      jumpUrl(sumform) {
        window.location.href = 'http://10.146.92.42:8080/qingbaoyun-navigation/fxpcgk/indorg/toexcel?a=' + sumform.aprop + '&b=' + sumform.btitle + '&type=' + this.type + '&title=' + this.title + '&token=' + this.userData.UserToken
      }
    },
    watch: {},
    mounted() {},
