  ...
  <div class="wrap-box">
    ...
    <div class="wrap-box__content">
      ...
      <el-button-group class="interactive">
        <el-button @click="praiseCollection((detail.stcomment!=='1' ? 1 : 2), 2)"><i :class="[detail.stcomment==='1'?'icon-like_xz':'']" class="iconfont icon-like_fill"></i> 收藏({{detail.collectionCount}})</el-button>
        <el-button @click="praiseCollection((detail.stPraise!=='1' ? 1 : 2), 1)"><i :class="[detail.stPraise==='1'?'icon-praise_xz':'']" class="iconfont icon-praise_fill"></i> 点赞({{detail.praiseCount}})</el-button>
      ...
      getDetail() {
        if (this.checkRule('zfsc/library/getinfo') === true) {
          ...
          this.dispatch(GET_TACTICS_DETAIL, {
            ...
            console.log(this.detail.stPraise)
      ...
      praiseCollection(flag, type) {
        if (this.checkRule('zfsc/dzsc/handle') === true) {
          this.dispatch(TACTICS_PRAISE_COLLECTION, {
            ...
            if (type === 1) {
              this.detail.stcomment = flag === 1 ? '1' : '0'
            }
            if (type === 2) {
              this.detail.stPraise = flag === 1 ? '1' : '0'
            }
          })
        }
      }
