...
<div style="height: 100%;">
  ...
  <tableList ref='tablelist' title="金融账户信息列表" :tab="table.jrzh" :list="pertionnelInfo" :pagenav="pageInfo" :edit="checkRule('fxpcgk/bankcount/update',basePeople)" :countLength="pageInfo.count" :readonly="readonly" :isEnd="isEnd" :remove="checkRule('fxpcgk/bankcount/delete',basePeople)"
...
import {
  ...
  FXPCGK_BANKCOUNT_INFO,
  FXPCGK_BANKCOUNT_SAVE,
  FXPCGK_BANKCOUNT_UPDATE,
  FXPCGK_BANKCOUNT_DELETE,
  FXPCGK_BANKCOUNT_LIST
} from '../../../../../store/types'
...
export default {
  ...
  methods: {
    queryList() {
      if(this.mainId) {
        // 查询列表
        this.dispatch(FXPCGK_BANKCOUNT_LIST, {
          fjid: this.mainId,
          sfz: this.IDcard,
          PageIndex: this.pageInfo.currentpage,
          PageSize: this.pageInfo.limit
        }).then(() => {
          this.pertionnelInfo = this.focusPerson.fxpcgkBankcountList.PageData
          this.pageInfo.count = this.focusPerson.fxpcgkBankcountList.DataCount
          this.isEnd = true
        })
    ...
    getQueryInfo(rowId, callback = () => {}) {
      this.dispatch(FXPCGK_BANKCOUNT_INFO, {
        id: rowId
      }).then(() => {
        callback(this.focusPerson.fxpcgkBankcountInfo)
      })
    },
    /**
     * 新增按钮事件
    ...
    remove(obj) {
      this.$confirm('确定要删除此项?', '温馨提示', {
        ...
        this.dispatch(FXPCGK_BANKCOUNT_DELETE, {
          ids: obj.row.id
        }, obj.index).then(() => {
          this.$message({
            message: '删除成功！',
    ...
    submit(form) {
      // 表单数据提交
      let submitType = FXPCGK_BANKCOUNT_SAVE
      if(form.id !== '') { // 判断表单id是否为空，非空可判断为更新提交
        submitType = FXPCGK_BANKCOUNT_UPDATE
      }
