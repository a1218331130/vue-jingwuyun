<template>
  <div>
      <el-form ref="car" :model="car" label-width="100px" :inline="true">
        ...
        <el-card class="carControl cars controlObject" v-loading="objLoading">
            ...
            <el-row>
              ...
              <el-col :span="6">
                ...
                <!-- <el-form-item prop="title" label="车辆类型">
                  <el-input v-model="car.jdccllxdm" required="true" placeholder="请输入车辆类型" :disabled="editFlag"></el-input>
                </el-form-item> -->
              </el-col>
            </el-row> 
             ...
             <el-row>
                  ...
                  <el-col :span="6" style="position: relative">
                    ...
                    <el-form-item prop="title" label="布控开始时间">
                      <el-date-picker type="date" :disabled="editFlag" placeholder="请选择布控开始时间" v-model="car.kssj" :picker-options="pickerOPtions"></el-date-picker>
                    </el-form-item>
                </el-col>
                  <el-col :span="6" style="position: relative">
                    <span style="color:red;position: absolute;left: 0px;top: 5px;">*</span>
                    <el-form-item prop="title" label="布控结束时间">
                      <el-date-picker type="date" :disabled="editFlag" placeholder="请选择布控结束时间" v-model="car.jssj" :picker-options="pickerOPtions2"></el-date-picker>
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row>
             ...
             <el-row>
                  <el-col :span="24">
                    <el-form-item prop="title" label="添加附件">
                      <upload @upload:submit="uploadSubmit" :currentFiles="currentFiles">
            ...
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="通知本人">
                  <el-radio-group :disabled="editFlag" v-model="car.sftzbr" @change="handleChangeNotice">
              ...
              <el-form-item label="已选择通知人" v-if="notifPerson.length >0">
                  <el-row v-for="(item, index) in notifPerson" :key="index">
                    <el-col :span="24" style="width:120%">
                      <span style="display:inline-block;width:50px">{{item.tzrxm}}</span>
        ...
        <el-row class="footerBtn">
              <el-button type="default" @click="resetForm('新增车辆布控')">
                取消
              </el-button>
              <el-button type="default" @click="saveCarMessage('2', '新增车辆布控')">
                保存草稿
              </el-button>
              <el-button type="primary" @click="saveCarMessage('5', '新增车辆布控')">
                提交
              </el-button>
...
<script>
  ...
  import { GET_CAR_INFO_LIST, GET_CAR_SAVE_CONTROL, GET_SYS_DICT_BY_TYPE } from '../../../store/types'
    ...
    methods: {
      ...
      getCarList(val) {
        ...
        this.dispatch(GET_CAR_INFO_LIST, {cphm: this.car.cphm, jdccllxdm: this.car.jdccllx}).then(() => {
          ...
          // console.log(this.dossier.CarInfoSB.length, 'this.dossier.CarInfoSB');
      ...
      saveCarMessage(val, title) {
        ...
        var data1 = new Date(this.car.kssj);
        var data2 = new Date(this.car.jssj);
        if(parseInt(data1.getTime()) > parseInt(data2.getTime())) {
          this.$message({
