  ...
  <div>
      <el-form ref="car" :model="car" label-width="100px" :inline="true" v-loading="editLoading">
        ...
        <el-card class="carControl cars controlObject" v-loading="objLoading">
            ...
            <!-- <el-row>
              <p v-if="carObjList.length===0" style="margin:0 auto;text-align:center">暂无该车信息</p>
            </el-row>  -->
         ...
         <el-card class="carControl cars carDetail">
            ...
            <div v-for="(list,iIndex) in checkList" :key='iIndex'>
              ...
              <div class="resultsAll" v-if="list.sqlx!=='1'">
                <div class="results" v-if="list.sqlx!=='3'"><span>续控时间截止：</span><span style="font-size:14px">{{list.jssj | dateFormat }}</span></div>
...
<script>
  ...
  import { GET_CAR_INFO_CONTROL, GET_CARRESULT_LIST_BY_BKID_CONTROL } from '../../../store/types'
    ...
    methods: {
      ...
      getEditObj() {
        ...
        this.dispatch(GET_CAR_INFO_CONTROL, {id: this.carId}).then(() => {
          ...
          // http://localhost:8001/#/mainIndex/3F95BA3849004CD89D2AC5F15B7B2A7D
          // http://10.146.92.92/#/controlManage/carControl
