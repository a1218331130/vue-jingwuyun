  ...
  <el-dialog v-draggable="dialogDraggableOptions" :visible.sync="dialog" :title="title||'家庭成员'"
    ...
    <el-form ref="editFormRef" :rules="editFormRules" :model="editForm" label-width="90px">
      <el-row :gutter="10" style="margin: 10px 20px 10px 0;">
        ...
        <el-col :span="8">
          <el-form-item label="关系" prop="relativeCode">
            <el-select placeholder="请选择与嫌疑人关系" v-model="editForm.relativeCode" filterable style="width: 100%">
      ...
      initDialog() {
        ...
        if (this.dataForm) {
          ...
          this.editForm = {
            ...
            name: _DF.name, // 姓名
            relative: this.dictLists['SARYGXDM'].filter(fr => fr.value === _DF.relativeCode).label, // 关系
            relativeCode: _DF.relativeCode, // 关系
      ...
      submit() {
        this.$refs['editFormRef'].validate(vt => {
          if (vt) {
            ...
            this.editForm.relative = this.editForm.relativeCode && this.dictLists['SARYGXDM'].filter(fr => fr.value === this.editForm.relativeCode)[0].label
