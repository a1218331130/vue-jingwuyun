...
<el-select placeholder="请选择同行人" v-model="editForm.associatedSuspect" filterable style="width: 100%">

              <el-option :label="item.name" :value="item.id" :key="index"
                         v-for="(item,index) in theSuspectList" style="width: 100%">
                         </el-option>
            </el-select>
  ...
  :rules="[{required: true, message: '必须输入证件号码'},{pattern: /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/, message: '你的身份证格式不正确', trigger: 'blur'}]">
  ...
  export default {
    ...
    data() {
      return {
        ...
        // 表单验证
        editFormRules: {
          associatedSuspect: [{ required: true, message: '不能为空' }],
          name: [
    ...
    methods: {
      ...
      initDialog() {
        ...
        if (this.isGangs && this.theSuspectList[0]) {
