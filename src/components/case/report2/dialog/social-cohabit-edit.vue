    ...
    <el-row :gutter="10" style="margin: 10px 20px 10px 0;">
      ...
      <el-col :span="12">
        <el-form-item label="证件号码" prop="IDCard">
          <el-input v-model="editForm.IDCard" placeholder="输入证件号码" :maxlength="18":rules="[{required: true, message: '必须输入证件号码'},{pattern: /(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/, message: '你的身份证格式不正确', trigger: 'blur'}]"></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item label="来本地日期" prop="toLocalTime">
          <el-date-picker v-model="editForm.toLocalTime" :editable="false" type="datetime" style="width: 100%" placeholder="请选择" :picker-options="disabledDateFun"></el-date-picker>
        </el-form-item>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item label="与房主关系" prop="houseOwnerRalation">
          <el-select placeholder="请选择与房主关系" v-model="editForm.houseOwnerRalation" filterable style="width: 100%">
            <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in dictLists['SARYGXDM']" style="width: 100%"></el-option>
          </el-select>
        </el-form-item>
      </el-col>
    </el-row>
    ...
    methods: {
      ...
      initDialog() {
        ...
        if (this.dataForm) {
          ...
          this.editForm = {
            ...
            houseOwnerRalationText: _df.yfzgxmc,
      ...
      submit() {
        this.$refs['editFormRef'].validate(vt => {
          if (vt) {
            this.editForm.houseOwnerRalationText = this.editForm.houseOwnerRalation && this.dictLists['SARYGXDM'].find(r => r.value === this.editForm.houseOwnerRalation).label
            // 确认提交
