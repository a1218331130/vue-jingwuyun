  ...
  <el-table :data="tableList" border>
    ...
    <el-table-column label="操作" width="80" align="center">
      <template scope="scope">
        <el-button type="text" @click="edit(scope.row)" title="编辑" v-if="checkUserRuleCase('ajgl/cyberbar/update', caseId) && scope.row.sourceCode !== '1'" icon="icon iconfont icon-brush"></el-button>
        <el-button type="text" title="删除" v-if="checkUserRuleCase('ajgl/cyberbar/delete', caseId)" @click="remove(scope.row,scope)" icon="icon iconfont icon-trash"></el-button>
      </template>
</el-table-column>
</el-table>
    ...
    methods: {
      ...
      submit(f) {
        ...
        if (f.id) {
          ...
          _optForm.id = '_cyberbar_' + new Date().getTime()
      ...
      localUpdate(ltableData, _text = null, types) {
        ...
        if (_tempReportContent) {
          _tempReportContent['tcyber'] = ltableData
        }
        this.close()
        this.updateReport({ type: 'tcyber', data: _tempReportContent['tcyber'], text: _text, modelType: types }) // 更新数据对象
      }
    },
    watch: {
      addSwitch(val) {
