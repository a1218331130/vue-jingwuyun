...
<el-table-column label="操作" width="80" align="center">
  <template scope="scope">
        <el-button type="text" @click="edit(scope.row)" title="编辑" v-if="checkUserRuleCase('ajgl/flight/update', caseId)  && scope.row.sourceCode !== '1' && scope.row.sourceCode !== 1" icon="icon iconfont icon-brush"></el-button>
        <el-button type="text" title="删除" v-if="checkUserRuleCase('ajgl/flight/delete', caseId) && scope.row.sourceCode !== '1' && scope.row.sourceCode !== 1" @click="remove(scope.row,scope)" icon="icon iconfont icon-trash"></el-button>
      </template>
</el-table-column>
</el-table>
  ...
  export default {
    ...
    methods: {
      ...
      submit(f) {
        ...
        // 判断是否来自于团伙操作
        let _optForm2 = {}
        console.log(this.theSuspect, 999);
        if (this.theSuspect) {
          console.log(this.theSuspect, 999999);
        ...
        if (f.id !== '') {
          ...
          _optForm.id = '_flight_' + new Date().getTime()
      ...
      localUpdate(FtableData, _text = null, types) {
        ...
        if (_tempReportContent) {
          _tempReportContent['tflight'] = FtableData
        }
        this.close()
        this.updateReport({ type: 'tflight', data: _tempReportContent['tflight'], text: _text, modelType: types }) // 更新数据对象
      }
    },
    watch: {
      addSwitch(val) {
