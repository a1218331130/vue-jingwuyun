            ...
            <div class="clearfix" slot="header">
             ...
             <el-row>
                 <el-col :span="6">
                  ...
                  <el-col :span="6" v-if="approveData.approvalStatus==='0'">
                     <el-form-item prop="caseName" label="指定协办人">
                       <el-select  v-model="xbr" placeholder="请选择" clearable filterable @change="xbrSelect">
                        <el-option v-for="item in xbrList" :label="item.applyUsername" :value="item.applyUserid" :key="item.applyUserid" required="true"></el-option>
                      </el-select> 
                    </el-form-item>
                  </el-col>
            </el-row>
...
<script>
  ...
  import { GET_APPLY_PERSON_INFO, GET_CUR_USER_CASE_LIST, XT_GET_CASE_INFO, XT_VIEW_DETAIL, XT_APPROVAL, GET_XBRLIST } from '../../../store/types'
    ...
    methods: {
      ...
      submitForm(title) {
        if(this.approveData.isAgree === '' || this.approveData.isAgree === undefined) {
          ...
          return;
        }else if(this.approveData.approvalStatus === '0') {
          if(this.xbr === '' || this.xbr === undefined) {
            this.$message({
              message: '请选择协办人',
              type: 'error'
            });
      ...
      xbrSelect(val) {
        this.approveData.approvalPersonid = val.split('#')[0];
        this.approveData.approvalPersonname = val.split('#')[1]
      },
      caseChange(val) {
      ...
      getXbrList() {
        this.dispatch(GET_XBRLIST).then(() => {
          this.xbrList = this.union.getXbrlist;
        })
      },
      getDetail(ids) {
    ...
    mounted() {
      if(this.xtTitles === '审批') {
        ...
        this.approveData = {
          ...
          approvalPersonid: '',
          approvalPersonname: '',
          approvalStatus: this.xtObjs.approvalStatus
        }
        // var ipList = location.href.split('/');
        // var myIp = ipList[2]
        this.editFlag = true;
