...
<div class="app-box-wrap">
  <div class="app-box">
    <!-- 列表 -->
    <el-table class="app-table" :show-header="listType===2" :height="tableHeight" v-loading.body="loading" :data="list" :border="true">
      <el-table-column v-if="listType===2" prop="number" label="案件编号"></el-table-column>
      ...
      <el-table-column v-if="listType===2" prop="typeText" label="报案类别"></el-table-column>
      <el-table-column v-if="listType===2" prop="stateText" label="案件状态"></el-table-column>
      <el-table-column v-if="listType===2" prop="meansText" label="作案手段"></el-table-column>
      <el-table-column v-if="listType===2" prop="area" label="发案地点"></el-table-column>
      <el-table-column v-if="listType===2" prop="peopleNumber" label="涉案人数"></el-table-column>
      <el-table-column v-if="listType===2" prop="facts" label="简要案情"></el-table-column>
      <el-table-column v-if="listType===2" prop="id" label="操作">
        <template scope="scope">
            <el-button type="text" size="small" @click="handleStudyCaseTo(scope.row.id)"
                       v-if="checkRule('ajgl/lawcase/info',true)">
              <i class="iconfont icon-iconfontsousuo1" title="查看"></i>
            </el-button>

            <el-button type="text" size="small" @click="remove(scope.row.id,scope.$index)"
                       v-if="checkRule('ajgl/lawcase/delete',true)">
                      ...
                      <el-row>
                        <el-col :span="24" class="name-box_elcol">
                          <div class="user-info-box_elrow">
                            <label>案件编号</label>
                            <span class="ID-card-text_span cursor--pointer"
                                  v-if="checkRule('ajgl/lawcase/info',true)"
                                  @click="handleStudyCaseTo(scope.row.id)">{{scope.row.number}}</span>
                            <span class="ID-card-text_span cursor--pointer"
                                  v-if="!checkRule('ajgl/lawcase/info',true)">{{scope.row.number}}</span>
                          ...
                          <span style="float: right">
                              <el-button class="import-btn"
                                         @click="handleStudyCaseTo(scope.row.id)"
                                         v-if="checkRule('ajgl/lawcase/info',true)"><i
                                class="iconfont icon-browse"></i>&nbsp;研判
                              </el-button>
                             <el-button class="import-btn delete" @click="remove(scope.row.id,scope.$index)"
                                        v-if="checkRule('ajgl/lawcase/delete',true)"><i
                               class="iconfont icon-trash"></i>&nbsp;删除
                            </el-button>
                            </span>
                        </el-col>
                        <el-col :span="24" class="name-box_elcol">
  ...
  methods: {
    ...
    queryList() {
      ...
      this.dispatch(FETCH_CASE_LIST, {
        data: Object.assign(this.queryCondition, {
    ...
    handleStudyCaseTo(cId) {
      ...
      this.jumpTo('/case/base?a=' + cId, true)
    },
    ...
    resizeHeight() {
      // 动态高度调整
      this.tableHeight = tableFill(this.$, '.app-box', false, this.$('.app-content').height() - 45)
    }
  },
  watch: {
    'caseInfo.PageData' (val) {
      if (val) {
        this.list = val.map(d => {
          return this.dataMatchs(d)
        })
        this.pageInfo.count = this.caseInfo.DataCount
        this.pageInfo.currentpage = this.caseInfo.PageIndex
        this.pageInfo.limit = this.caseInfo.PageSize
      }
    },
    queryCondition: {
