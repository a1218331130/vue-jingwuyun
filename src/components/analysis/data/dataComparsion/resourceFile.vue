        ...
        <div v-for="(item,index1) in relationConditionArr" :key="index1">
          ...
          <div>
            <el-select v-model="item.FieldName1" placeholder="请选择" filterable @change="getLeftRequirement">
              <el-option v-for="(o1,index2) in selectConditionArr[0]" :key="index2" :label="o1.NameCn" :value="o1.ID"></el-option>
            </el-select>
            <span>=</span>
            <el-select v-model="item.FieldName2" placeholder="请选择" filterable @change="getRightRequirement">
              <el-option v-for="(o2,index3) in selectConditionArr[1]" :key="index3" :label="o2.NameCn" :value="o2.Name"></el-option>
            </el-select>
...
<script>
  ...
  import { FETCH_GET_ZYML, FETCH_GET_TABLE_INFOL, FETCH_MY_DATA_LIST, FETCH_WDBD_DATA_DETAIL } from '../../../../store/types'
    ...
    methods: {
      ...
      initTableInfo() {
        ...
        this.dispatch(FETCH_MY_DATA_LIST, { Keyword: '', PageIndex: 1, PageSize: 100, SourceType: '' }).then(() => {
          ...
          this.$nextTick(() => {
            this.dispatch(FETCH_WDBD_DATA_DETAIL, { ID: ID }).then(() => {
              this.checkedArr = [this.dataComparison.GetWdbdDataDetail.ID]
              this.tableName = [{ name: this.dataComparison.GetWdbdDataDetail.Name, count: this.dataComparison.GetWdbdDataDetail.Count }]
              this.radio = 0
              this.getRemoteConditions(this.dataComparison.GetWdbdDataDetail.ID, (data) => {
                this.$set(this.selectConditionArr, '0', data)
              })
              this.comparisonData.SjbdTableModels = [{
                IsMaster: 1,
                TableIndex: 0,
                TableID: this.dataComparison.GetWdbdDataDetail.ID,
                TableType: 2
              }]
            })
          })
        ...
        this.relationConditionArr.forEach((e, i) => {
          this.selectConditionArr[0].forEach((el, j) => {
            if (e.FieldName1 === el.ID) {
              // this.$delete(e, 'NameCn1')
              // this.relationConditionArr[i].FieldName1 = el.ID;
