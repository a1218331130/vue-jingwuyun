...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      upStep(step) {
        this.$emit('step', step)
        bus.$emit('firstRest', null)
      },
      downStep(step) {
        this.nextComparisonData.SjbdTableFilterModels = this.filterList[0].parameter.concat(this.filterList[1].parameter)
        this.$emit('step', step);
        this.$nextTick(() => {
          bus.$emit('acossSecondComparisonData', this.nextComparisonData);
          bus.$emit('filterNumber', this.nextComparisonData.SjbdTableFilterModels.length)
        })
      },
      close(done) {
        this.dialog = false
            ...
            this.gatherArr.forEach(m => {
              if (this.value === n.Name) {
                if (this.value1 === m.prop) {
                  ...
                  this.filterList[1].parameter.push({ TableIndex: this.index, FieldName: this.value, Val: this.value2, TableID: this.filterList[1].tableID, OperatorType: this.value1 });
                }
              }
            })
          })
    ...
    created() {
      bus.$on('acossFilterList', n => {
        this.filterList = n
      })
      bus.$on('acossFirstComparisonData', m => {
