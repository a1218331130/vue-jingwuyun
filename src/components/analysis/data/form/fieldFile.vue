...
<script>
  ...
  export default {
    ...
    created() {
      bus.$on('formFilterList', m => {
        this.data1 = m;
      })
      bus.$on('formSecondComparisonData', n => {
        this.comparisonData = n;
      })
      this.$connect.on('ImportSjbdDataProcess', (data) => {
        if (data.ID) {
          console.log('收到 ImportSjbdDataProcess 推送, 开始回调处理');
          this.importPercentage = parseInt((data.CompletedCount / data.Total) * 100) || 100
