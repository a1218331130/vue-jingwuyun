...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      initCount() {
        let SjbdTableModels1 = [],
          formData1
        SjbdTableModels1[0] = this.comparisonData.SjbdTableModels[0]
        formData1 = {
          SourceType: 4,
      ...
      upStep(step) {
        this.$emit('step', step)
        bus.$emit('formfirstRest', null)
      },
      downStep(step) {
        this.comparisonData.SjbdTableFilterModels = this.filterList[0].parameter
        this.$emit('step', step);
        this.$nextTick(() => {
          bus.$emit('formSecondComparisonData', this.comparisonData)
          bus.$emit('formfilterNumber', this.comparisonData.SjbdTableFilterModels.length)
        })
      },
      close(done) {
        this.dialog = false
    ...
    created() {
      bus.$on('formFilterList', n => {
        this.filterList = n;
      })
      bus.$on('formFirstComparisonData', m => {
        this.comparisonData = m;
        this.initCount()
      })
    },
    mounted() {}
