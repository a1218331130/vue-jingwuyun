    ...
    <div style="height:calc(100vh - 60px)">
      ...
      <div class="result-list" ref="list">
        <xdh-scroll :on-reach-bottom="handleReachBottom" :distance-to-edge="-150" :height="listHeight" :total="total" :personList="['123']">
          <wechat-item :dataList="elem" v-if="total!==0" v-for="(elem,i) in telList" :key="i"></wechat-item>
        </xdh-scroll>
        <!-- 结果为空 -->
...
import wechatItem from './item/wechat-item.vue'
...
export default {
  ...
  data() {
    return {
      listHeight: 500,
      type: '1',
      tabs: [{ label: '手机搜微信', value: '1' }, { label: '微信搜手机', value: '2' }], // tab类型
      inputValue: '', // 查询关键字
      keyword: '', // 关键字
      total: 0, // 结果条数
      telList: [], // 结果列表
