    ...
    <div style="height:calc(100vh - 60px)">
      ...
      <div class="result-list" ref="list">
        <xdh-scroll :on-reach-bottom="handleReachBottom" :distance-to-edge="-150" :height="listHeight" :total="total" :personList="['123']">
          <wbtel-item :dataList="elem" v-if="total!==0" v-for="(elem,i) in telList" :key="i"></wbtel-item>
        </xdh-scroll>
        <!-- 结果为空 -->
...
import wbtelItem from './item/wbtel-item.vue'
...
export default {
  ...
  data() {
    return {
      listHeight: 500,
      type: '1',
      tabs: [{ label: '手机搜微博', value: '1' }, { label: '微博搜手机', value: '2' }], // tab类型
      inputValue: '', // 查询关键字
      keyword: '', // 关键字
      total: 0, // 结果条数
      telList: [], // 结果列表
