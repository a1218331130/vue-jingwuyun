            ...
            <el-form :model="item.queryForm" :inline="true" label-position="right" label-width="90px">
              ...
              <el-form-item label="同乘规则">
                <el-select v-model="item.queryForm.takeRule" placeholder="请选择" class="small">
                  <el-option v-for="item in takeList" :label="item.label" :value="item.value"
                             :key="item.value"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="同乘次数">
                >=
                <el-input-number class="small" v-model="item.queryForm.callCount" size="small" :min="1"
                                 :max="1000"></el-input-number>
              </el-form-item>
              <el-form-item label="车站条件">
                <el-select class="small" v-model="item.queryForm.stationRule" placeholder="请选择">
                  <el-option v-for="item in stationList" :label="item.label" :value="item.value"
                             :key="item.value"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="前后时间偏移" labelWidth="120px">
                <el-input-number v-model="item.queryForm.num" size="small" :min="1" :max="1000"
                                 class="small"></el-input-number>
                分钟
              </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        stationList: [{value: 1, label: '出发地和目的地相同'}, {value: 2, label: '出发地相同'}, {value: 3, label: '目的地相同'}, {
          value: 4,
          label: '不限'
        }],
        table: {
