    ...
    <div class="pgis-sidebar-wrap">
      ...
      <div class="poi-list-wrap">
        <ul class="poi-list" v-if="searchResult">
          <li v-for="(item, index) in searchResult.pois" :key="index"
          @mouseenter="handleMouseEnter(item, index)" @mouseleave="handleMouseLeave(item, index)" @click="handleItemClick(item, index)" :class="{'is-selected':selected && item.id === selected.id}">
...
<script>
  ...
  export default {
    ...
    watch: {
      'Location.jd': {
        handler: function() {
          let newVal = this.Location
          if (newVal) {
            this.openInfo([newVal.jd, newVal.wd], newVal.marker);
            this.map.animationTo(newVal.coord, 'pan', {
              duration: 1000
            });
            this.$emit('update:currentMarker', newVal.marker)
          }
        },
        deep: true
      },
      'clickLocation.x': {
        handler: function() {
          let newVal = this.clickLocation
          if (newVal) {
            this.openInfo([newVal.x, newVal.y], newVal.marker);
            this.map.animationTo(newVal.coord, 'pan', {
              duration: 1000
            });
            this.$emit('update:currentMarker', newVal.marker)
    ...
    methods: {
      ...
      createMarker(index, poi, flag) {
        const position = new window.EzCoord(poi.x, poi.y);
        let src = ''
        if (!flag) {
          src = '/static/num/red.png'
        } else {
          src = `/static/num/red${index % 10}.png`
        }
      ...
      renderMarker(pois, flag = true) {
        this.markers.forEach(marker => {
          this.map.removeOverlay(marker);
        })
        this.markers = []
        pois.forEach((poi, index) => {
          this.createMarker(index, poi, flag)
        })
      },
      getMarkerAtPixel(e) {
        let pixel = e.pixel
    ...
    mounted() {
      ...
      this.map.addMapEventListener(window.Ez.Event.MAP_CLICK, e => {
        let coord = e.coordinate,
          coordObj = { x: coord[0], y: coord[1] },
          marker = this.getMarkerAtPixel(e)
        if (this.clickMarkerMode === true) {
          this.renderMarker([coordObj])
          this.$emit('update:newMarker', coordObj)
        } else {
