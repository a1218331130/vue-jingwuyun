...
<el-dialog v-draggable="dialogDraggableOptions" :visible.sync="dialog" :title="type==='person'?'选择人员':'选择车辆'" :modal-append-to-body="true" :append-to-body="true" :size="size||'large--fixed'" :modal="size!=='full'" :close-on-click-modal="false" @open="initDialog"
  ...
  <el-row>
    <el-col :span="6">
      <el-tree class="cat-tree" v-if="tree.length>0" :data="tree" node-key="id" :highlight-current="true" :indent="0" :default-expand-all="true" @node-click="nodeClick"></el-tree>
    </el-col>
    <el-col :span="18">
      ...
      <div class="list-wrap" v-if="this.type==='car'">
        ...
        <div class="list-box">
          <el-table ref="objectTable" @cell-mouse-enter="cellMouseEnter" class="app-table" :data="list2" :show-header="true" :border="true" v-loading="loading" height="370" @select="updateSelection" @select-all="allSelection" style="overflow:auto">
            <el-table-column type="selection" width="55"></el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      updatePage(data) {
        ...
        // this.getImptrantPerson(data.currentpage, data.limit)
      },
