...
<transition>
  <div>
    ...
    <el-dialog v-draggable="dialogDraggableOptions" :title="imageList.name" v-if="dialogVisible.isShow" v-model="dialogVisible.isShow" :modal="false" size="full">
      <img width="100%" height="100%" :src="dialogVisible.ImageUrl" alt="" />
      <div slot="footer" class="dialog-footer">
        <el-button type="" @click="closeEvent">关闭</el-button>
        <a :href="imageList.url" :download="imageList.name" :title="imageList.name">
...
export default {
  ...
  methods: {
    ...
    handleSuccess(res, file, fileList) {
      ...
      if(file.raw.type.split('/')[0] === 'video') { // 判断上传文件是否为视频文件,特殊处理 获取相关信息
        ...
        _tFileList = fileList.map(df => {
          if(df.response) {
            if(df.response.Data.ID === file.response.Data.ID) {
              df.url = defaultFileIcon // 上传视频图片预览
            }
          }
          return df
        })
    ...
    handlePreview(file) {
      // console.log(file, 'handlePreview', this.uploadType === 'picture' && !this.isSinglePic, this.uploadType);
      if(this.uploadType === 'picture' && !this.isSinglePic) {
        if(file.name.endsWith('.bmp') || file.name.endsWith('.png') || file.name.endsWith('.jpg') || file.name.endsWith('.gif') || file.name.endsWith('.jpeg')) {
          this.dialogVisible.ImageUrl = file.url
          this.imageList = file
          this.dialogVisible.isShow = true
        } else {
          this.imageList = file
          if(!file.url.includes('/static/img/fileTypeIcon')) {
