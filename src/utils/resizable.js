 ...
 *
 */
import {addClass, removeClass, getStyle, setStyle} from '../../node_modules/element-ui/src/utils/dom'
...
// 拖拽容器的className
const RESIZABLE_CLASS = 'el-resizable';
...
class Resizable extends Base {
  ...
  init(options) {
    ...
    if (!o.disabled) {
      ...
      this.bind(this.el, 'mousedown', this.handleMouseDown);
      this.bind(this.el, 'mouseleave', this.handleMouseLeave);
      this.bind(this.el, 'mouseenter', this.handleMouseEnter);
  ...
  setCursor(dir) {
    ...
    setStyle(this.el, 'cursor', cursor);
  }

  destroy() {
    this.unbind(this.el, 'mousedown', this.handleMouseDown);
    this.unbind(this.el, 'mouseleave', this.handleMouseLeave);
    this.unbind(this.el, 'mouseenter', this.handleMouseEnter);
